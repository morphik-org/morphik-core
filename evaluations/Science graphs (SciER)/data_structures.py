...
def get_sentence_of_span(span, sentence_starts, doc_tokens):
    ...
    in_between = [
        span[0] >= start and span[1] <= end for start, end in zip(sentence_starts, sentence_ends)
    ]
...
class Document:
    def __init__(self, js):
        self._doc_key = js["doc_id"]
        entries = fields_to_batches(
            js, ["doc_id", "doc_tokens", "clusters", "predicted_clusters", "section_starts"]
        )
        ...
        self.sentences = [
            ...
            for sentence_ix, (entry, sentence_start) in enumerate(zip(entries, sentence_starts))
        ]
        if "clusters" in js:
        ...
        if "predicted_clusters" in js:
            self.predicted_clusters = [
                Cluster(entry, i, self) for i, entry in enumerate(js["predicted_clusters"])
            ]

    def __repr__(self):
        return "\n".join(
            [str(i) + ": " + " ".join(sent.text) for i, sent in enumerate(self.sentences)]
        )
...
class NER:
    ...
    def __eq__(self, other):
        return self.span == other.span and self.label == other.label and self.flavor == other.flavor
...
class Events:
    ...
    def matches_except_event_type(self, argument):
        matched = [
            candidate
            for candidate in self.span_matches(argument)
            if candidate.event_type != argument.event_type and candidate.role == argument.role
        ]
        return set(matched)
