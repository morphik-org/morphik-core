...
class GitHubConnector(BaseConnector):
    ...
    async def get_auth_status(self) -> ConnectorAuthStatus:
        if not self.ee_settings.GITHUB_CLIENT_ID or not self.ee_settings.GITHUB_CLIENT_SECRET:
            return ConnectorAuthStatus(is_authenticated=False, message="GitHub OAuth credentials not configured")

        if self.client:
            try:
                # Test the connection
                self.client.get_user().login
                return ConnectorAuthStatus(is_authenticated=True, message="Successfully connected to GitHub")
            except Exception as e:
                logger.error(f"GitHub connection test failed for user {self.user_morphik_id}: {e}")

        return ConnectorAuthStatus(is_authenticated=False, message="Not authenticated with GitHub")
    ...
    async def initiate_auth(self) -> Dict[str, Any]:
        ...
        return {"authorization_url": auth_url, "state": self.user_morphik_id}
    ...
    def _escape_xml_content(self, content: str) -> str:
        ...
        if len(escaped) > 50000 or any(ord(c) < 32 and c not in "\t\n\r" for c in content[:1000]):
    ...
    async def pack_repository_smart_chunking(
        self, repo_path: str, chunk_by_type: bool = True, max_chunk_size: int = 100000
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            def categorize_file(path: str) -> str:
                ...
                if ext in source_extensions:
                    ...
                    if any(
                        name in path_lower
                        for name in ["readme", "license", "changelog", "contributing", "authors", "credits"]
                    ):
                        return "docs"
                    elif any(
                        name in path_lower
                ...
                for item in contents:
                    if item.type == "file":
                        ...
                        try:
                            ...
                            files.extend(
                                get_all_files(sub_contents if isinstance(sub_contents, list) else [sub_contents])
                            )
                        except Exception as e:
            ...
            for file_item in all_files:
                try:
                    ...
                    chunks[category]["files"].append(
                        {"path": file_item.path, "size": file_item.size, "sha": file_item.sha}
                    )
        ...
        try:
            ...
            if not force:
                try:
                    ...
                    if existing_docs:
                        logger.warning(
                            f"Repository {repo_path} already exists with {len(existing_docs)} documents. Skipping duplicate ingestion. Use force=true to re-ingest."
                        )
            ...
            chunks = await self.pack_repository_smart_chunking(
                repo_path=repo_path, chunk_by_type=True, max_chunk_size=100000  # Larger chunks for ingestion
            )
