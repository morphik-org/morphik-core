...
class ChatMessage(BaseModel):
    ...
    role: str = Field(..., description="The role of the message author. Can be 'system', 'user', or 'assistant'.")
    content: str = Field(..., description="The content of the message.")
    name: Optional[str] = Field(None, description="The name of the author of this message.")
    function_call: Optional[Dict[str, Any]] = Field(None, description="The name and arguments of a function that should be called")
...
class ChatCompletionChoice(BaseModel):
    ...
    finish_reason: Optional[str] = Field(None, description="The reason the chat completion finished.")
...
class CreateChatCompletionRequest(BaseModel):
    ...
    presence_penalty: Optional[float] = Field(0.0, description="Penalty for new tokens based on their presence in text so far.")
    frequency_penalty: Optional[float] = Field(0.0, description="Penalty for new tokens based on their frequency in text so far.")
    logit_bias: Optional[Dict[str, float]] = Field(None, description="Modify likelihood of specified tokens appearing.")
    user: Optional[str] = Field(None, description="A unique identifier representing your end-user.")

class CreateChatCompletionResponse(BaseModel):
    """Response from creating a chat completion."""
    id: str = Field(..., description="A unique identifier for the chat completion.")
    object: Literal["chat.completion"] = Field("chat.completion", description="The object type (always 'chat.completion').")
...
class CreateCompletionRequest(BaseModel):
    ...
    presence_penalty: Optional[float] = Field(0.0, description="Penalty for new tokens based on their presence in text so far.")
    frequency_penalty: Optional[float] = Field(0.0, description="Penalty for new tokens based on their frequency in text so far.")
    best_of: Optional[int] = Field(1, description="Generate best_of completions server-side and return the best.")
    logit_bias: Optional[Dict[str, float]] = Field(None, description="Modify likelihood of specified tokens appearing.")
    user: Optional[str] = Field(None, description="A unique identifier representing your end-user.")
...
class CreateCompletionResponse(BaseModel):
    ...
    object: Literal["text_completion"] = Field("text_completion", description="The object type (always 'text_completion').")
    created: int = Field(default_factory=lambda: int(time.time()), description="The Unix timestamp of when the completion was created.")
    model: str = Field(..., description="The model used for completion.")
...
class EmbeddingData(BaseModel):
    ...
    object: Literal["embedding"] = Field("embedding", description="The object type (always 'embedding').")
