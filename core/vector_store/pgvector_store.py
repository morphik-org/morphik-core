...
class PGVectorStore(BaseVectorStore):
    ...
    async def get_chunk(self, chunk_id: str) -> Optional[DocumentChunk]:
        ...
        The chunk_id is expected to be in the format: document_id-chunk_number
        """
        try:
            if "-" not in chunk_id:
                logger.error(f"Invalid chunk ID format: {chunk_id}")
                return None
                
            document_id, chunk_number_str = chunk_id.rsplit("-", 1)
            try:
                chunk_number = int(chunk_number_str)
            except ValueError:
                logger.error(f"Invalid chunk number in chunk ID: {chunk_id}")
                return None
                
            async with self.async_session() as session:
                query = select(VectorEmbedding).where(
                    VectorEmbedding.document_id == document_id,
                    VectorEmbedding.chunk_number == chunk_number
                )
